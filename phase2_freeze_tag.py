"""
phase2_freeze_tag.py  –  Phase 2: Interacting Agents (Freeze Tag)
=================================================================
GAME RULES
----------
• Prey   (blue)   uses A* to navigate toward the Reward each turn.
• Monster(red)    uses A* to chase the Prey, recalculating every turn.
...
-----------------------
Both agents run A* (informed search) every step:
  - Prey    : A*( prey_pos  → reward_pos )   — fixed goal
  - Monster : A*( monster_pos → prey_pos  )  — goal changes every step

Each game-tick both agents advance ONE tile along their current path.
If either A* path is empty the agent stays put (already at goal / blocked).

INTEGRATION
-----------
Drops into the existing AI_Zombie_Project folder.
Requires: config.py  and  utils.py  in the parent directory (same layout as
          phase2_astar.py / phase2_search.py).

Run:
    python phase2_freeze_tag.py
"""
